    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            <div class="flex items-center space-x-4 mb-8 pb-6 border-b border-gray-100">
                ...
                <div>
                    <h2 class="text-xl font-bold text-gray-800">Editar Documento</h2>
                    <p class="text-sm text-gray-500 mt-1">Modifique la información del documento</p>
                </div>
            </div>

            <form action="{{ route('documentos.update', $documento) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative group">
                            <select id="tipo" name="tipo"
                                ...
                                <option value="Copia" {{ old('tipo', $documento->tipo) == 'Copia' ? 'selected' : '' }}>Copia</option>
                                <option value="Formulario" {{ old('tipo', $documento->tipo) == 'Formulario' ? 'selected' : '' }}>Formulario</option>
                                <option value="Carta" {{ old('tipo', $documento->tipo) == 'Carta' ? 'selected' : '' }}>Carta</option>
                                <option value="Comprobante" {{ old('tipo', $documento->tipo) == 'Comprobante' ? 'selected' : '' }}>Comprobante</option>
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative group">
                            <input type="date" id="fecha_expiracion" name="fecha_expiracion"
                                   class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-200 rounded-lg focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 transition-all group-hover:border-[#4F46E5]/50"
                                   value="{{ old('fecha_expiracion', $documento->fecha_expiracion->format('Y-m-d')) }}"
                                   required>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative group">
                            <select id="es_visible" name="es_visible"
                                    class="block w-full px-4 py-2.5 text-gray-700 bg-white border border-gray-200 rounded-lg focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 transition-all group-hover:border-[#4F46E5]/50"
                                    required>
                                <option value="1" {{ old('es_visible', $documento->es_visible) ? 'selected' : '' }}>Visible</option>
                                <option value="0" {{ old('es_visible', $documento->es_visible) ? '' : 'selected' }}>No Visible</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                <i class="fas fa-chevron-down text-gray-400"></i>
                            </div>
